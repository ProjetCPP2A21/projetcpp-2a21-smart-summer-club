...
void service::statistic_capacite()
{
    QSqlQuery query;
    if (!query.exec(R"(
        SELECT s.type_service,
               s.capacite,
               COUNT(b.id_apprenant) AS nbBeneficiaires
        FROM service s
        LEFT JOIN beneficier b ON s.id_service = b.id_service
        GROUP BY s.type_service, s.capacite
    )")) {
        qDebug() << "Erreur SQL:" << query.lastError().text();
        return;
    }
    ...
    QBarSet *setBeneficiaires = new QBarSet("Bénéficiaires");
    QStringList categories;

    while (query.next()) {
        QString serviceName = query.value(0).toString();
        int capacite = query.value(1).toInt();
        int nbBeneficiaires = query.value(2).toInt();

        *setCapacite << capacite;
        *setBeneficiaires << nbBeneficiaires;
        categories << serviceName;

        qDebug() << "Service:" << serviceName
                 << "Capacité:" << capacite
                 << "Beneficiaires:" << nbBeneficiaires;
    }

    if (setCapacite->count() == 0) {
        qDebug() << "⚠️ Aucune donnée valide pour le graphique.";
        return;
    }

    series->append(setCapacite);
    series->append(setBeneficiaires);

    QChart *chart = new QChart();
    chart->addSeries(series);
    chart->setTitle("nombre inscrit par service");
...
bool service::alert_capacite(int id_service)
{
    ...
    if (!trayIcon.isVisible()) {
        trayIcon.setIcon(QIcon(":/icon/warning.png"));
        trayIcon.show();
    }
