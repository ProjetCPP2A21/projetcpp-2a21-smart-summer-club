...
*/
    /********************arduino service_apprenant ET buzzer***************************/
    ...
    case -1: // Le cas -1 n'était pas présent dans connect_arduino() mais est géré ici.
        ...
        QMessageBox::warning(this, "Arduino non connecté",
                             "Aucun Arduino n'est connecté.\n"
                             "La vérification sonore des équipements ne fonctionnera pas.");
        break;
    }
    ...
    /******************** Connecter le lineEdit de vérification buzzer ***************************/
    // Connecter le lineEdit de vérification
    connect(ui->verifBuzzer, &QLineEdit::returnPressed,
            this, &interface_formateur::on_lineEdit_verifBuzzer_returnPressed);

    // Connecter le bouton de vérification
    connect(ui->pushbutton_verifBuzzer, &QPushButton::clicked,
            this, &interface_formateur::on_pushbutton_verifBuzzer_clicked);

    // Connecter le changement de texte
    connect(ui->verifBuzzer, &QLineEdit::textChanged,
           this, &interface_formateur::on_lineEdit_verifBuzzer_textChanged);
    /****************************************************************************/
...
void interface_formateur::on_pushButton_AJOUTER_5_clicked()
{
    ...
    QDate dateEmbauche = ui->dateEdit_embaucheFormateur->date();
    QString sexe;
...
void interface_formateur::on_pushButton_listeformateur_2_clicked()
{
    ...
    ui->doubleSpinBox_heureprevueFormateur->setMinimum(0.0);
    ui->doubleSpinBox_heureprevueFormateur->setMaximum(50.0);
    ui->doubleSpinBox_heureprevueFormateur->setSingleStep(0.25);
}
...
// ========== VÉRIFICATION ÉQUIPEMENT AVEC BUZZER ==========

bool interface_formateur::verifierEquipementParID(const QString &idEquipement)
{
    if (idEquipement.isEmpty()) {
        return false;
    }
    ...
    if (!query.exec()) {
        qDebug() << "[BUZZER] Erreur SQL lors de la vérification de l'équipement:" << query.lastError().text();
        return false;
    }
    ...
    if (query.next()) {
        ...
        if (count > 0) {
            // Équipement trouvé - récupérer plus d'informations
            query.prepare("SELECT NOM, ETAT, QUANTITE FROM EQUIPEMENT WHERE ID = :id");
            query.bindValue(":id", idEquipement);
            ...
            if (query.exec() && query.next()) {
                ...
                qDebug() << "[BUZZER] Équipement trouvé - ID:" << idEquipement
                         << "Nom:" << nom
                         << "État:" << etat
                         << "Quantité:" << quantite;
            ...
            qDebug() << "[BUZZER] Équipement NON TROUVÉ - ID:" << idEquipement;
            return false;
        }
    }
...
void interface_formateur::on_pushbutton_verifBuzzer_clicked()
{
    QString idEquipement = ui->verifBuzzer->text().trimmed();

    if (idEquipement.isEmpty()) {
        QMessageBox::warning(this, "Champ vide", "Veuillez saisir un ID d'équipement !");
        return;
    }

    qDebug() << "[BUZZER] Vérification de l'équipement ID:" << idEquipement;

    // Vérifier l'équipement dans la base de données
    bool equipementValide = verifierEquipementParID(idEquipement);

    // Contrôler le buzzer selon le résultat
    // Utiliser l'Arduino 'd' qui est déjà connecté
    if (d.getserial() && d.getserial()->isOpen() && d.getserial()->isWritable()) {
        if (equipementValide) {
            // Équipement valide : un bip court
            QByteArray command = "B1\n";
            d.write_to_arduino(command);
            qDebug() << "[BUZZER] Signal sonore envoyé: B1 (bip court)";

            // Afficher message et changer le style
            QMessageBox::information(this, "✅ Équipement Valide",
                                     QString("L'équipement ID %1 est valide !\nBip court envoyé au buzzer.")
                                         .arg(idEquipement));
            ui->verifBuzzer->setStyleSheet("background-color: #d4edda; color: #155724; border: 2px solid #c3e6cb;");
        } else {
            // Équipement invalide : trois bips successifs
            QByteArray command = "B3\n";
            d.write_to_arduino(command);
            qDebug() << "[BUZZER] Signal sonore envoyé: B3 (trois bips)";

            // Afficher message et changer le style
            QMessageBox::warning(this, "❌ Équipement Invalide",
                                 QString("Aucun équipement trouvé avec l'ID: %1\nTrois bips envoyés au buzzer.")
                                     .arg(idEquipement));
            ui->verifBuzzer->setStyleSheet("background-color: #f8d7da; color: #721c24; border: 2px solid #f5c6cb;");
        }

        // Effacer le champ après un délai
        QTimer::singleShot(1500, this, [this]() {
            ui->verifBuzzer->clear();
            ui->verifBuzzer->setStyleSheet("");
        });

    } else {
        qDebug() << "[BUZZER] Arduino non connecté, impossible d'envoyer le signal sonore";
        QMessageBox::warning(this, "Arduino non connecté",
                             "L'Arduino n'est pas connecté ou n'est pas prêt.\n"
                             "Vérifiez la connexion USB et redémarrez l'application.");
    }
}

void interface_formateur::on_lineEdit_verifBuzzer_returnPressed()
{
    // Appeler la même fonction que le bouton lorsqu'on appuie sur Entrée
    on_pushbutton_verifBuzzer_clicked();
}

void interface_formateur::on_lineEdit_verifBuzzer_textChanged(const QString &text)
{
...
// ========== FIN VÉRIFICATION ÉQUIPEMENT AVEC BUZZER ==========
