...
#include "mainwindow.h"  // ✅ Add this include
#include <QMessageBox>
#include <QDebug>
#include <QSqlRecord>
    ...
    , ui(new Ui::gestion_employe)
{
    ...
    connect(ui->pushButton_gestionemploye_2, &QPushButton::clicked, this, [this]() {
...
void gestion_employe::on_pushButton_se_connecter_clicked()
{
    ...
    if (etmp.seConnecter(id, mdp)) {
        ...
        // ✅ Get the user's poste
        QString poste = etmp.getPosteById(id);

        if (poste.isEmpty()) {
            QMessageBox::warning(this, "Erreur", "Impossible de récupérer le poste de l'utilisateur.");
            return;
        }

        // Create and show MainWindow
        MainWindow *mainWin = new MainWindow();
        mainWin->setPosteEmploye(poste);  // ✅ Set the poste
        mainWin->show();

        // Close the current gestion_employe window
        this->close();
    } else {
...
/* ==================== RECHERCHE ARDUINO ==================== */
void gestion_employe::on_btRechercheArduino_clicked()
{
    QString idText = ui->lineEdit_idArduino->text().trimmed();
    etmp.rechercherArduino(arduino, idText);
}
