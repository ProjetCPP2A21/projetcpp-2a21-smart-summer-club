...
void Formateur::setSmsSender(SmsSender *sender)
{
    smsSender = sender;
}
...
{
    // 1) Récupérer l'ancien salaire
    QSqlQuery queryOld;
    QString IdF = QString::number(IdFormateur);
    queryOld.prepare("SELECT SALAIRE, CONTACT FROM FORMATEUR WHERE IDFORMATEUR = :Id");
    queryOld.bindValue(":Id", IdF);

    double ancienSalaire = 0;
    QString contactFormateur;
    if(queryOld.exec() && queryOld.next()) {
        ancienSalaire = queryOld.value("SALAIRE").toDouble();
        contactFormateur = queryOld.value("CONTACT").toString();
    }
    QString num = "216"+contactFormateur;
    ...
    // 3) Envoyer un SMS si le salaire a changé
    if(success && ancienSalaire != SalaireFormateur) {
        SmsSender *sms = new SmsSender();
        QString message = QString("**********EDUCAMP***********\nBonjour %1 %2,\n votre salaire a été modifié: %3 -> %4")
                              .arg(NomFormateur)
                              .arg(PrenomFormateur)
                              .arg(ancienSalaire)
                              .arg(SalaireFormateur);
...
QSqlQueryModel * Formateur :: afficher()
{
    ...
    model ->setQuery("SELECT IDFORMATEUR, NOM, PRENOM, CONTACT, SEXE, DATE_EMBAUCHE, SPECIALITE, NB_HEURES_PREVUES, SALAIRE FROM FORMATEUR");
...
{
    QSqlQuery query;
    query.prepare("SELECT IDFORMATEUR, NOM, PRENOM, CONTACT, SEXE, DATE_EMBAUCHE, SPECIALITE, NB_HEURES_PREVUES, SALAIRE FROM FORMATEUR WHERE IDFORMATEUR = :id");
...
{
    QSqlQueryModel * model = new QSqlQueryModel();
    model->setQuery("SELECT IDFORMATEUR, NOM, PRENOM, CONTACT, SEXE, DATE_EMBAUCHE, SPECIALITE, NB_HEURES_PREVUES, SALAIRE FROM FORMATEUR ORDER BY DATE_EMBAUCHE ASC");
...
void Formateur::exporterPDF()
{
    ...
    // Données
    QSqlQuery query;
    query.prepare("SELECT IDFORMATEUR, NOM, PRENOM, CONTACT, SEXE, DATE_EMBAUCHE, SPECIALITE, NB_HEURES_PREVUES, SALAIRE FROM FORMATEUR FORMATEUR");
    if (query.exec()) {
        while (query.next()) {
...
QImage Formateur::generateQrCode() const
{
    if (IdFormateur <= 0){
        qDebug() << "Erreur : IdFormateur n'est pas défini ou invalide :" << IdFormateur;
    }
    ...
    QImage img(size * scale, size * scale, QImage::Format_RGB888);
    img.fill(Qt::white);
            ...
            if (qr.getModule(x, y)) {
                for (int dy = 0; dy < scale; dy++)
                    for (int dx = 0; dx < scale; dx++)
                        img.setPixelColor(x * scale + dx, y * scale + dy, Qt::black);
            }
        }
    }
...
bool Formateur::recherche_arduino(QString id)
{
    QSqlQuery query;
...
bool Formateur::donnee_arduino(int id, QString &nom, QString &prenom)
{
    QSqlQuery query;
    query.prepare("SELECT NOM, PRENOM FROM FORMATEUR WHERE IDFORMATEUR = :id");
    query.bindValue(":id", id);
